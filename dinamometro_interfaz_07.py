# -*- coding: utf-8 -*-
"""
Created on Mon May 30 17:45:01 2022
...
def hilo_interfaz(args):
    ...
    def hilo_lectura_datos(args):
        ...
        canal_lectura_digital = 0                           # Canal a usar para leer frecuencia.
        ...
        while getattr(hilo_1, "do_run", True):
            ...
            for chan in canales_lectura:
                if chan == 0:
                    value = modulo_mcc128.a_in_read(chan, opciones)
                    mostrar_voltaje_celda_01.set('{:.5f}'.format(value))
                    lista_celda_01.append(mostrar_voltaje_celda_01.get())
                elif chan == 1:
                    value = modulo_mcc128.a_in_read(chan, opciones)
                    mostrar_voltaje_celda_02.set('{:.5f}'.format(value))
                    lista_celda_02.append(mostrar_voltaje_celda_02.get())
                elif chan == 4:
                    value = modulo_mcc128.a_in_read(chan, opciones)
                    mostrar_voltaje_excitacion.set('{:.5f}'.format(value))
                    lista_excitacion.append(mostrar_voltaje_excitacion.get())
            ...
            i=0
            tiempo_muestreo = time.time()
            while (time.time()-tiempo_muestreo)<0.0250:
                valor_digital = modulo_mcc152.dio_input_read_bit(canal_lectura_digital)
                print(valor_digital)
...
if __name__ == "__main__":
    ...
    root.title("Pruebas Dinamómetro V0.7") #Titulo de la ventana principal

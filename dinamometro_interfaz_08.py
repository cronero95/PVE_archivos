# -*- coding: utf-8 -*-
"""
Created on Tue May 31 14:06:09 2022
        ...
        while getattr(hilo_1, "do_run", True):
            ...
            for chan in canales_lectura:
                if chan == 0:
                    ...
                    mostrar_voltaje_celda_01.set('{:.5f}'.format(value, ' V'))
                    lista_celda_01.append(mostrar_voltaje_celda_01.get().replace(' V', ''))
                elif chan == 4:
                    value = modulo_mcc128.a_in_read(chan, opciones)
                    mostrar_voltaje_celda_02.set('{:.5f}'.format(value, ' V'))
                    lista_celda_02.append(mostrar_voltaje_celda_02.get().replace(' V', ''))
                elif chan == 1:
                    value = modulo_mcc128.a_in_read(chan, opciones)
                    mostrar_voltaje_excitacion.set('{:.5f}'.format(value, ' V'))
...
if __name__ == "__main__":
    ...
    root.title("Pruebas Dinamómetro V0.8") #Titulo de la ventana principal
